...
std::unique_ptr<ImageProvider> MainLoop::getImageProvider() {
...
void MainLoop::normalizeMat(const cv::Mat& in, cv::Mat& out) {
    cv::Scalar avg, sdv;
...
void MainLoop::dumpPpm(ofstream& fout, const cv::Mat& frame) {
    if (fout.is_open()) {
        vector<uchar> buff;
...
void MainLoop::writeEstHeader(ofstream& fout) {
    fout << "Frame" << "\t"
         << "Id" << "\t"
         << "Label" << "\t"
         << "Lid" << "\t"
...
void MainLoop::dumpEst(ofstream& fout, GazeHypsPtr gazehyps) {
    if (fout.is_open()) {
...
void MainLoop::setHorizGazeTolerance(double tol)
{
    horizGazeTolerance = tol;
}

void MainLoop::setVerticalGazeTolerance(double tol)
{
    verticalGazeTolerance = tol;
}

void MainLoop::setSmoothing(bool enabled)
{
    smoothingEnabled = enabled;
}


void MainLoop::interpretHyp(GazeHyp& ghyp) {
